...
impl MockNamedPipePair {
    ...
    pub async fn write_message(stream: &mut DuplexStream, msg: &IpcMessage) -> Result<(), String> {
        let payload =
            rmp_serde::to_vec_named(msg).map_err(|e| format!("MsgPack serialize error: {e}"))?;
    ...
    pub async fn read_message(stream: &mut DuplexStream) -> Result<IpcMessage, String> {
        ...
        let msg =
            rmp_serde::from_slice(&buf).map_err(|e| format!("MsgPack deserialize error: {e}"))?;
        Ok(msg)
    }
}
...
mod tests {
    ...
    async fn test_mock_named_pipe_framing_roundtrip() {
        ...
        MockNamedPipePair::write_message(&mut pair.client_stream, &msg)
            .await
            .unwrap();
        let received = MockNamedPipePair::read_message(&mut pair.server_stream)
            .await
            .unwrap();
