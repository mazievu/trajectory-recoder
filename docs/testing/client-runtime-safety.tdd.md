...
| Capture rejects a server role and uses the configured spool | `cargo test -p capture-agent capture_runtime_rejects_non_client_role_and_uses_configured_spool` | Failed to compile because `config::ClientRuntimeConfig` did not exist. | Passed with the shared file-backed client configuration loader. |
